...
class Screen:
    ...
    def __init__(self, screenname, screen_cfg, display, drawinstance):
        ...
        self.disable_probing_for_all_streams = screen_cfg.setdefault(
        ...
        logger.debug("Screen: " + self.name +
                     " duration from config is: " + str(self.duration))
    ...
    def _init_camera_streams(self):
        ...
        for camera_stream in self.camera_streams_cfg:
            ...
            cam_stream = CameraStream(
                cam_stream_name, camera_stream, self.drawinstance, self.display_vlc_hdmi_id)
    ...
    def update_connectable_camera_streams(self, skip=False):
        ...
        if skip:
            logger.debug("Screen: " + self.name +
                         " Skipping checking connectable cameras, skip value is: " + str(skip))
            ...
            logger.debug(
                "Screen: Start polling connectivity for the camera_streams part of screen: " + self.name)
    ...
    def set_layer(self, layer):
        self.layer = layer
        logger.debug("Screen: layer for screen " + self.name +
                     " has been set to " + str(self.layer))

    def get_layer(self):
        return self.layer

    def reset_layer(self):
        logger.debug("Screen: layer for screen " + self.name + " will be reset to " +
                     str(self.layer_init_value) + " coming from " + str(self.layer))
        self.layer = self.layer_init_value
    ...
    def reset_active_timer(self):
        ...
        logger.debug("Screen: " + self.name +
                     " start_of_active_time: " + str(self.start_of_active_time))
    ...
    def get_active_run_time(self):
        ...
        logger.debug("Screen: " + self.name + " active_run_time: " +
                     str(active_run_time) + " / " + str(self.duration))
    ...
    def update_screen(self):
        ...
        if self._is_connectable_streams_changed() or len(self.previous_connectable_camera_streams) == 0:
            ...
            logger.debug("Screen: " + self.name +
                         " number of fields= " + str(fields))

            if fields == 0:
                # Draw no connectable placeholder
                self.drawinstance.placeholder(
                    0, 0, self.resolution_width, self.resolution_height, "images/noconnectable.jpg")
            ...
            for cam_stream in self.cam_streams_to_draw:
                ...
                if currentwindow == fields:
                    ...
                    if free_horizontal_pixels > 0:
                        logger.debug("Screen: We have " + str(free_horizontal_pixels) +
                                     " free_horizontal_pixels unused screen. Start drawing placeholders to fill the free space")
                        ...
                        while count_placeholders < nr_of_placeholders:
                            self.drawinstance.placeholder(
                                placeholder_x, placeholder_y, normal_fieldwidth, normal_fieldheight, "images/placeholder.jpg")
