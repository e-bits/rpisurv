...
class CameraStream:
    ...
    def __init__(self, name, camera_stream, drawinstance, display_hdmi_id):
        ...
        self.force_coordinates = camera_stream.setdefault(
            "force_coordinates", False)
        self.network_caching_ms = camera_stream.setdefault(
            "network_caching_ms", 500)
        self.freeform_advanced_vlc_options = camera_stream.setdefault(
        ...
        if self.imageurl and self.showontop:
            logger.error(
                f"CameraStream: {self.name} is an imageurl which does not support showontop")
        ...
        self.cvlc_extra_options = self.cvlc_extra_options + \
            ' ' + self.freeform_advanced_vlc_options
        self.parsed = urlparse(self.url)
        self.port = self.parsed.port
        self.scheme = self.parsed.scheme
        self.hostname = self.parsed.hostname
        self.obfuscated_credentials_url = '{}://{}'.format(
        ...
        if self.scheme == "rtsp":
            ...
            self.rtsp_options_cmd = "OPTIONS " + self._manipulate_credentials_in_url(
                "remove") + " RTSP/1.0\r\nCSeq: 1\r\nUser-Agent: rpisurv\r\nAccept: application/sdp\r\n\r\n"

        self.obfuscated_credentials_url = self._manipulate_credentials_in_url(
            "obfuscate")

        if self.scheme not in ["rtsp", "http", "https", "file", "rtmp"]:
            logger.error("CameraStream: " + self.name + " Scheme " + self.scheme + " in " + self.obfuscated_credentials_url +
                         " is currently not supported, you can make a feature request on https://community.rpisurv.net")
            sys.exit()
    ...
    def _manipulate_credentials_in_url(self, action):
        ...
        if self.parsed.password is not None or self.parsed.username is not None:
            ...
            if action == "obfuscate":
                obfuscated = self.parsed._replace(
                    netloc='<hidden_username>:<hidden_password>@{}'.format(host_info))
                return obfuscated.geturl()
            elif action == "remove":
                no_credentials = self.parsed._replace(
                    netloc='{}'.format(host_info))
    ...
    def _urllib2open_wrapper(self):
        ...
        if self.parsed.password is not None and self.parsed.username is not None:
            ...
            request = urllib.request.Request(
                strippedcreds_url.geturl(), None, headers)
        ...
        if self.scheme == "rtmp":
            try:
                ffprobeoutput = subprocess.check_output(
                    ['/usr/bin/ffprobe', '-v', 'quiet', "-print_format", "flat", "-show_error", self.url], text=True, timeout=self.probe_timeout)
                return True
            except subprocess.TimeoutExpired as e:
                logger.error(
                    f"CameraStream: {self.name} {self.obfuscated_credentials_url} Not Connectable (ffprobe timed out, try increasing probe_timeout for this stream), configured timeout: {self.probe_timeout}")
                return False
            except Exception as e:
                erroroutput_newlinesremoved = e.output.replace('\n', ' ')
                logger.error(
                    f"CameraStream: {self.name} {self.obfuscated_credentials_url} Not Connectable ({erroroutput_newlinesremoved}), configured timeout: {self.probe_timeout}")
                return False
        if self.scheme == "rtsp":
            try:
                ...
                logger.error("CameraStream: " + self.name + " " + str(self.obfuscated_credentials_url) +
                             " Not Connectable (failed socket connect), configured timeout: " + str(self.probe_timeout) + " " + repr(e))
                return False
            ...
            if not rtsp_response:
                logger.error("CameraStream: " + self.name + " " + str(self.obfuscated_credentials_url) +
                             " Not Connectable (failed rtsp validation, no response from rtsp server)")
                return False

            if re.match("RTSP/1.0", rtsp_response.decode('utf-8').splitlines()[0]):
                logger.debug("CameraStream: " + self.name + " " +
                             str(self.obfuscated_credentials_url) + " Connectable")
                return True
            else:
                logger.error("CameraStream: " + self.name + " " + str(self.obfuscated_credentials_url) +
                             " Not Connectable (failed rtsp validation, is this an rtsp stream?)")
                return False
            ...
            try:
                ...
                if connection.getcode() == 200:
                    ...
                    logger.error("CameraStream: " + self.name + " " + str(self.obfuscated_credentials_url) +
                                 " Not Connectable (http response code: " + str(connection.getcode()) + ")")
                    return False
                connection.close()
            except urllib.error.URLError as e:
                logger.error("CameraStream: " + self.name + " " + str(
                    self.obfuscated_credentials_url) + " Not Connectable (URLerror), " + repr(e))
                return False
                ...
                logger.error("CameraStream: " + self.name + " " + str(
                    self.obfuscated_credentials_url) + " Not Connectable (" + repr(e) + " )")
                return False
        elif self.scheme == "file":
            if os.path.isfile(self.parsed.path):
                logger.debug(
                    f"CameraStream: {self.name} {self.parsed.path} file found")
                return True
            else:
                logger.error(
                    f"CameraStream: {self.name} {self.parsed.path} file not found")
                return False
        else:
            logger.error("CameraStream: " + self.name + " Scheme " + str(self.scheme) + " in " + str(self.obfuscated_credentials_url) +
                         " is currently not supported, you can make a feature request on https://community.rpisurv.net")
            sys.exit()

    def calculate_field_geometry(self):
        self.normal_fieldwidth = int(self.coordinates[2] - self.coordinates[0])
        self.normal_fieldheight = int(
            self.coordinates[3] - self.coordinates[1])

    def show_status(self):
        self.calculate_field_geometry()
        self.drawinstance.placeholder(
            self.coordinates[0], self.coordinates[1], self.normal_fieldwidth, self.normal_fieldheight, "images/connecting.jpg")
        ...
        if self.imageurl:
            ...
            if self.is_connectable():
                try:
                    ...
                    self.calculate_field_geometry()
                    self.drawinstance.placeholder(
                        self.coordinates[0], self.coordinates[1], self.normal_fieldwidth, self.normal_fieldheight, self.image_file)
                except Exception as e:
                    # Do not crash rpisurv if there is something wrong with loading the image at this time
                    logger.error("CameraStream: This stream " +
                                 self.name + " refresh_image_from_url " + repr(e))
        else:
            logger.debug("CameraStream: This stream " + self.name +
                         " is not an imageurl, skip refreshing imageurl")

    def start_stream(self, coordinates, layer):
        if self.force_coordinates:
            logger.debug("CameraStream: This stream " + self.name + " uses force_coordinates " + str(
                self.force_coordinates) + " which will override pre-calculated coordinates of " + str(coordinates))
        ...
        if self.showontop:
            logger.debug(
                f"CameraStream: Start stream on top  of the other streams {self.name}")
            self.layer = layer + 1
        else:
            self.layer = layer
        logger.debug("CameraStream: Start stream " +
                     self.name + " on layer " + str(self.layer))
    ...
    def stop_stream(self):
        ...
        if not self.imageurl:
            ...
            if self.stopworker is not None:
                ...
                logger.debug("CameraStream: MAIN Value of stopworker for " +
                             self.name + " is " + str(self.stopworker.value))

                # Wait for the worker to be terminated before continuing https://github.com/SvenVD/rpisurv/issues/84
                logger.debug(
                    "CameraStream: Executing join for stream " + self.name)
                self.worker.join()
