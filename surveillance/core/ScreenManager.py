...
class ScreenManager:
    ...
    def __init__(self, screen_manager_name, display, enable_opportunistic_caching_next_screen, disable_pygame):
        ...
        if self.disable_pygame:
            ...
            self.drawinstance.insert_black_background(int(self.display["resolution"]["width"]), int(
                self.display["resolution"]["height"]), self.display["display_number"])
    ...
    def _fetch_display_config(self):
        ...
        with open(configpath, 'r') as ymlfile:
            ...
            self.disable_autorotation = cfg['essentials'].setdefault(
    ...
    def _create_cached_screen(self):
        ...
        if self.all_screens[self.futurecacheindex].get_layer() < self.lowerthreshold_layer:
            logger.debug(
                f"{self.name}: skip caching next screen: {self.all_screens[self.futurecacheindex].name} once, because we need to reset the layer counter")
        ...
        if self.get_future_cached_screen_disable_probing_for_all_streams():
            ...
            self.all_screens[self.futurecacheindex].update_connectable_camera_streams(
                skip=True)
        else:
            self.all_screens[self.futurecacheindex].update_connectable_camera_streams(
                skip=False)
        ...
        if self.enable_opportunistic_caching_next_screen and not self.skip_cache_once:
            ...
            logger.debug(
                f"{self.name}: Skip caching screen {self.all_screens[self.futurecacheindex].name} because of parameters enable_opportunistic_caching_next_screen: {self.enable_opportunistic_caching_next_screen} / skip_cache_once: {self.skip_cache_once}")
        ...
        if self.futurecacheindex > self.max_index:
            logger.debug(
                f"{self.name}: futurecacheindex ({self.futurecacheindex} >= {self.max_index}), resetting futurecacheindex")
    ...
    def force_show_screen(self, requested_index):
        ...
        logger.debug(
            f"{self.name}: activeindex = {self.activeindex}, futurecacheindex = {self.futurecacheindex}")
        if requested_index > self.max_index:
            # Note name of screens start with 1 but list of screens index start at 0
            logger.debug(
                f"{self.name}: Force screen {requested_index + 1} requested, but this screen does not exist")
        elif requested_index == self.activeindex:
            # Note name of screens start with 1 but list of screens index start at 0
            logger.debug(
                f"{self.name}: Force screen {requested_index + 1} requested, but this screen is already active")
        elif requested_index == self.currentcacheindex:
            # Note name of screens start with 1 but list of screens index start at 0
            logger.debug(
                f"{self.name}: Force screen {requested_index + 1} requested, this screen is already in cache, do a normal rotate")
            self.rotate_next()
        else:
            # Note name of screens start with 1 but list of screens index start at 0
            logger.debug(
                f"{self.name}: Force screen {requested_index + 1} requested")
            ...
            self.all_screens[requested_index].update_connectable_camera_streams(
                skip=True)
            ...
            if self.futurecacheindex > self.max_index:
                logger.debug(
                    f"{self.name} force screen futurecacheindex ({self.futurecacheindex} >= {self.max_index}), resetting futurecacheindex ")
                self.futurecacheindex = 0
    ...
    def rotate_next(self):
        ...
        if self.max_index == 0:
            logger.debug(
                f"{self.name}: only one screen configured, do not rotate")
        ...
        if self.firstrun:
            ...
            logger.debug(
                f"{self.name} starting first run active screen {self.all_screens[self.activeindex].name}")
            self.all_screens[self.activeindex].reset_active_timer()
            self._create_cached_screen()
            self.firstrun = False
            ...
            logger.debug(
                f"{self.name}: destroying previous screen {self.all_screens[self.activeindex].name}")
            self.all_screens[self.activeindex].destroy()
            # Now the cached screen should already be displayed because the active on top was destroyed
            logger.debug(
                f"{self.name}: next screen: {self.all_screens[ self.currentcacheindex].name} active")
            ...
            logger.debug(
                f"{self.name}: rotate event, indexes AFTER rotate: futurecacheindex: {self.futurecacheindex} activeindex: {self.activeindex} max index is {self.max_index}")
    ...
    def _init_screens(self):
        ...
        for screen_cfg in self.screens_cfg:
            logger.debug(
                f"{self.name}: Initialising screen with config {screen_cfg}")
            self.all_screens.append(Screen(str(
                self.name) + "_screen" + str(counter), screen_cfg, self.display, self.drawinstance))
            counter = counter + 1
        ...
        self.drawinstance.placeholder(0, 0, int(self.display["resolution"]["width"]), int(
            self.display["resolution"]["height"]), "images/connecting.jpg")
    ...
    def _init_drawinstance(self):
        self.drawinstance = Draw([int(self.display["resolution"]["width"]), int(
            self.display["resolution"]["height"])], self.disable_pygame, self.name)
    ...
    def update_active_screen(self, skip_update_connectable_camera=False):
        ...
        logger.debug(
            f"{self.name}: update_active_screen {self.all_screens[self.activeindex].name}")
        if self.get_active_screen_disable_probing_for_all_streams():
            ...
            self.all_screens[self.activeindex].update_connectable_camera_streams(
                skip=True)
            logger.debug(
                f"{self.name}: SKIPPING update_connectable_camera_streams, because disable_probing_for_all_streams for this screen {self.all_screens[self.activeindex].name} was set")
        else:
            self.all_screens[self.activeindex].update_connectable_camera_streams(
                skip=False)
            logger.debug(
                f"{self.name}: update_connectable_camera_streams, disable_probing_for_all_streams is off for this screen, so using probes {self.all_screens[self.activeindex].name}")
